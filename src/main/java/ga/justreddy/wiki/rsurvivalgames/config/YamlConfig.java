package ga.justreddy.wiki.rsurvivalgames.config;
...
public class YamlConfig {
    ...
    public YamlConfig(final String name)
        ...
        final File configFile = new File(RSurvivalGames.getInstance().getDataFolder().getAbsolutePath(), completeName);

        if (!configFile.exists()) {
            //noinspection ResultOfMethodCallIgnored
            configFile.getParentFile().mkdirs();
            RSurvivalGames.getInstance().saveResource(completeName, false);
        }

        this.file = configFile;
        this.config = YamlConfiguration.loadConfiguration(file);
        reload();
        ChatUtil.sendConsole("&7[&dRSurvivalGames&7] &aLoaded config: " + name);
    }
    ...
    public boolean isOutdated(final int currentVersion) {
        if (config.getInt(VERSION_KEY, -1) < currentVersion) {
            ChatUtil.sendConsole("&7[&dRSurvivalGames&7] &cConfiguration file " + file.getName() + " is outdated! Shutting down plugin.");
